...
- 代理设置: socks5://127.0.0.1:10808 (通过 v2ray)
- 主分支: main

**多电脑同步说明：**
1. Clone 仓库到新电脑：\`git clone <repo-url>\`
2. 配置 Git 用户信息（如不同）：\`git config --global user.name "Your Name"\`
3. 配置 GitHub 代理（如在国内）：
   \`\`\`bash
...
详细配置说明请查看 [.opencode/README.md](.opencode/README.md)

**全局配置：** \`~/.config/opencode/opencode.json\`
- oh-my-opencode 插件：已安装
- MCP 服务器：配置在全局，不提交到 Git
...
\`\`\`bash
# 在 OpenCode 会话中
@agent 使用 error-diagnostic 技能分析这个错误
@agent 使用 github-explorer 探索相关项目
\`\`\`

详细技能配置请查看 [.opencode/README.md](.opencode/README.md)
...
Ultrawork 是 Oh-My-OpenCode 提供的超强工作模式。只需在提示词中添加 \`ulw\` 或 \`ultrawork\` 关键词，即可激活：
...
| **Explore** | 快速代码库探索和上下文 grep | GLM-4.7 |
